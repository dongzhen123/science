...
__global__ void  Da(float *da_next2,float *da,int sum)
{
   const int tid=threadIdx.x;
   const int bid=blockIdx.x;
   for(int u=tid+bid*THREAD_NUM;u<sum;u+=BLOCK_NUM*THREAD_NUM)
   {
     da_next2[u]=da_next2[u]+da[u];
    }
}
...
{  //[gradients,Allerror]=LSTM(train_x{num,1},train_y{num,1},parameters);
    ...
    for(int t=1;t<=T_x;t++){
        ...
        cublasSgemm(handle,CUBLAS_OP_N,CUBLAS_OP_N,4*n_a2,m,n_a1,&alpha,w2_a1, 4*n_a2, a1+t*n_a1*m,n_a1,&beta,output_3,4*n_a2);
        ...
        cublasSgemm(handle,CUBLAS_OP_N,CUBLAS_OP_N,n_y,m,n_a2,&alpha,wy, n_y, a2+t*n_a2*m,n_a2,&beta,output_5,n_y);
    ...
    cudaMalloc((void**)&door1,4*n_a1*m*sizeof(float)); 
     ...
     Da<< <BLOCK_NUM,THREAD_NUM>> >(d_a2+t*n_a2*m,da+(t-1)*n_a2*m,n_a2*m);
     ...
     cublasSgemm(handle,CUBLAS_OP_T,CUBLAS_OP_N,n_a1,m,4*n_a2,&alpha,w2_a1, 4*n_a2,door2, 4*n_a2,&beta1,d_a1+t*n_a1*m,n_a1);

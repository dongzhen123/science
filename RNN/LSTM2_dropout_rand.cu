...
__global__ void dropout(float *a_dropout,float *a,float *Dropout,float *dropout,int sum,float drop)
{
   const int tid=threadIdx.x;
   const int bid=blockIdx.x;
...
__global__ void Add(float *a,float *by,int n_y,int m)                        //soft=wy*a_next2+repmat(by,1,m)
{
   const int tid=threadIdx.x;
   const int bid=blockIdx.x;
...
__global__ void sum(float *a,float *b,int n_y,int m)
{
 ...
 //soft-repmat(max(soft),n_y,1)-repmat(log(sum(exp(soft-repmat(max(soft),n_y,1)))),n_y,1)
...
{  //[gradients,Allerror]=LSTM2_dropout_rand(x(:,kk((num-1)*m+1:num*m),:),y(kk((num-1)*m+1:num*m),:),parameters);


    const size_t *dim_array = mxGetDimensions(prhs[0]);
	int n_x=*dim_array,m=*(dim_array+1),T_x=*(dim_array+2);
    int n_a1=256,n_a2=256,n_y=9;
    ...
    //输入数据（x,y,w）拷贝到GPU
    float *x_t,*y_t,*dropout1,*dropout2,*Dropout1,*Dropout2,*a1_dropout,*a2_dropout,*a_backdropout;
    float drop1=1.0,drop2=0.8;
    ...
    curandGenerateUniform(gen,dropout1, n_a1*m*T_x);
    curandGenerateUniform(gen,dropout2, n_a2*m*T_x);
    ...
    float *output_1,*output_2,*output_3,*output_4,*output_5,*output_7,*output_8,*output_9,*output_10;
    ...
    cudaMalloc((void**)&output_7,m*sizeof(float)); 
    cudaMalloc((void**)&output_8,n_y*m*sizeof(float)); 
    ...
    float *output_diff,*da,*error,*error_cpu=(float*)malloc(m*sizeof(float));
    ...
    for(int t=1;t<=T_x;t++){
        ...
        dropout<< <BLOCK_NUM,THREAD_NUM>> >(a1_dropout,a1+t*n_a1*m,Dropout1+(t-1)*n_a1*m,dropout1+(t-1)*n_a1*m,n_a1*m,drop1);
        ...
        dropout<< <BLOCK_NUM,THREAD_NUM>> >(a2_dropout,a2+t*n_a2*m,Dropout2+(t-1)*n_a2*m,dropout2+(t-1)*n_a2*m,n_a2*m,drop2);

        cublasSgemm(handle,CUBLAS_OP_N,CUBLAS_OP_N,n_y,m,n_a2,&alpha,wy, n_y, a2_dropout,n_a2,&beta,output_5,n_y);

        Add<< <BLOCK_NUM,THREAD_NUM>> >(output_5,by,n_y,m);                     //soft=wy*a_next2+repmat(by,1,m)
        Max<< <BLOCK_NUM,THREAD_NUM>> >(output_5,output_7,n_y,m);               //max(soft)
        soft_repmat<< <BLOCK_NUM,THREAD_NUM>> >(output_5,output_7,output_8,output_9,n_y,m);   

        //soft-repmat(max(soft),n_y,1)-repmat(log(sum(exp(soft-repmat(max(soft),n_y,1)))),n_y,1)

        sum<< <m,THREAD_NUM>> >(output_9,output_10,n_y,m);
        log_softmax<< <BLOCK_NUM,THREAD_NUM>> >(output_8,output_10,output_diff+(t-1)*n_y*m,y_t+(t-1)*m,error,n_y*m,n_y);
    ...
    cudaMalloc((void**)&door1,4*n_a1*m*sizeof(float)); 
     ...
     /*
     const size_t dim22[]={1,10};
     plhs[2] = mxCreateNumericArray(2,dim22 ,mxSINGLE_CLASS, mxREAL);
     cudaMemcpy((float*)mxGetPr(plhs[2]),dropout1,10*sizeof(float), cudaMemcpyDeviceToHost);
     const size_t dim33[]={1,10};
     plhs[3] = mxCreateNumericArray(2,dim33 ,mxSINGLE_CLASS, mxREAL);
     cudaMemcpy((float*)mxGetPr(plhs[3]),dropout2,10*sizeof(float), cudaMemcpyDeviceToHost);
     */

...
cnn = {
    struct('layer', 'i') 
    struct('layer', 'c', 'w',w1,'b',b1,'strides',[1 1],'activation','Relu','padding','SAME') %32*32*32
    ...
    %16*16*32
    struct('layer', 'c', 'w',w3,'b',b3,'strides',[1 1],'activation','Relu','padding','SAME')%16*16*64
    struct('layer', 'c', 'w',w4,'b',b4,'strides',[1 1],'activation','Relu','padding','SAME') 
    struct('layer', 's', 'pool','max','ksize',[2 2],'strides',[2 2],'padding','VALID')
    %8*8*64
    struct('layer', 'c', 'w',w5,'b',b5,'strides',[1 1],'activation','Relu','padding','SAME')%8*8*128
    struct('layer', 'c', 'w',w6,'b',b6,'strides',[1 1],'activation','Relu','padding','SAME')
    struct('layer', 's', 'pool','max','ksize',[2 2],'strides',[2 2],'padding','VALID','pkeep',0.8) %4*4*128

...
__global__ void OutputMatrix(float *Res_In,float *dev,float *output,int L_rows,int L_cols,int R_cols)
{
...
subB[row][col]=dev[L_cols*(blockCol*blocksize+col)+row+i*blocksize];
else
subB[row][col]=0;
